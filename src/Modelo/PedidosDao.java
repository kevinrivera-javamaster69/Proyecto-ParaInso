...
public class PedidosDao {
    ...
    public int verificarStado(int box, int id_ambiente){
        int id_pedido = 0;
        String sql = "SELECT id FROM pedidos WHERE num_box=? AND id_ambiente=? AND estado = ?";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            ps.setInt(1, box);
    ...
    public int RegistrarPedido(Pedidos ped){
        String sql = "INSERT INTO pedidos (id_ambiente, num_box, total, usuario) VALUES (?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            ps.setInt(1, ped.getId_ambiente());
            ps.setInt(2, ped.getNum_box());
    ...
    public Pedidos verPedido(int id_pedido){
       ...
       String sql = "SELECT p.*, s.nombre FROM pedidos p INNER JOIN ambientes s ON p.id_ambiente = s.id WHERE p.id = ?";
       try {
           con = cn.getConnection();
           ps = con.prepareStatement(sql);
           ps.setInt(1, id_pedido);
           rs = ps.executeQuery();
            if (rs.next()) {               
               ...
               ped.setAmbiente(rs.getString("nombre"));
               ped.setNum_box(rs.getInt("num_box"));
    ...
    public void pdfPedido(int id_pedido) {
        ...
        try {
            ...
            doc.open();
            //Image img = Image.getInstance(getClass().getResource("/Iconos/logo.jpg"));
            //Fecha
            String informacion = "SELECT p.*, s.nombre FROM pedidos p INNER JOIN ambientes s ON p.id_ambiente = s.id WHERE p.id = ?";
            try {
                ps = con.prepareStatement(informacion);
                ps.setInt(1, id_pedido);
                rs = ps.executeQuery();
                if (rs.next()) {
                    num_box = rs.getString("num_box");
                    ambiente = rs.getString("nombre");
    ...
    public List listarPedidos(){
       ...
       String sql = "SELECT p.*, s.nombre FROM pedidos p INNER JOIN ambientes s ON p.id_ambiente = s.id ORDER BY p.fecha DESC";
       try {
           con = cn.getConnection();
           ps = con.prepareStatement(sql);
           rs = ps.executeQuery();
